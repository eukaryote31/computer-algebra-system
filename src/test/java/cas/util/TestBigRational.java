...
public class TestBigRational {
	...
	public void testIsZero() {
		// common sense
		assertTrue(BigRational.build(0).isZero());
		assertFalse(BigRational.build(1).isZero());
		
		// rollover
		assertFalse(BigRational.build(Integer.MAX_VALUE).isZero());
		assertFalse(BigRational.build(1L + (long) Integer.MAX_VALUE).isZero());
	...
	public void testIsPositive() {
		...
		assertFalse(BigRational.build("0").isPositive());

		assertFalse(BigRational.build(-100).isPositive());
		assertTrue(BigRational.build(100).isPositive());
	}
	
	@Test
	public void testIsNegative() {
		// zero is not negative
		assertFalse(BigRational.build("0").isNegative());

		assertFalse(BigRational.build(100).isNegative());
		assertTrue(BigRational.build(-100).isNegative());
	}
	
	@Test
	public void testReciprocal() {
		assertEquals(BigRational.build("9/7"), BigRational.build("7/9").reciprocal());
	}
}
